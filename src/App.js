...
import backgroundMusic from "./bg.mp3"; // Import your MP3 file here
...
const GameTimer = ({ gameStart, seconds, setSeconds }) => {
  useEffect(() => {
    ...
    if (gameStart) {
      console.log("আমি এখানে আয়না ঘরে ছিলাম!");
  ...
  return (
    <div>
      <h2 className="game-timer">Saved from শত্রুজ: {seconds} টা Khaleda</h2>
    </div>
  );
};
...
const CollMesage = () => {
  return <div className="coll-message">পালাবি কোথায়!</div>;
};
...
const App = () => {
  ...
  const gameStartBtn = () => {
    ...
    audioRef.current.play();  // Start playing the music when the game starts
  };
  ...
  const gameStopBtn = () => {
    setGameStart(false);
    audioRef.current.pause();  // Stop the music when the game stops
      ...
      enemyMoving = setInterval(() => {
        ...
        enemyRefs.current.forEach((enemyRef, i) => {
          ...
          if (
            ...
            setCollision(true);
            setGameStart(false);
            audioRef.current.pause();  // Stop the music on collision
          }
        });
      }, 100);
    }
  ...
  return (
    <>
      <audio ref={audioRef} src={backgroundMusic} loop />  {/* Audio element */}
